...
export default function BuscarPessoa({ navigation }) {
  ...
  async function BuscarPessoa() {
    ...
    const BuscarPessoa = query(produtoRef, where("NomePessoa", "==", busca)); // vai fazer a pergunta ao banco do euseEste PASSO 3 query é uma busca
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Busque usuário aqui:</Text>
      <TextInput
        style={styles.input}
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Text>
            Nome da pessoa: <Text>{item.NomePessoa}</Text>
          </Text>
        )}
      />
      <FlatList
        ...
        renderItem={({ item }) => (
          <Text>
            Idade da pessoa: <Text>{item.IdadePessoa}</Text>
          </Text>
        )}
      />
      <FlatList
        ...
        renderItem={({ item }) => (
          <Text>
            Apelido da pessoa: <Text>{item.ApelidoPessoa}</Text>
          </Text>
        )}
      />
    </View>
